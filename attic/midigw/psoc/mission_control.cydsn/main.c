...
// Analog3
#define MISSION_CONTROL_ID 0x700
#define MODULE_ID_BASE 0x701
...
void handle_can_message(uint32_t id, uint8_t is_extended, uint8_t is_remote, uint8_t len, uint8_t *data)
{
    uint8_t local_buf[8];
    if (is_extended && !is_remote && len > 0) {
        switch (data[0]) {
        case A3_ADMIN_REQUEST_ID: {
            SERIAL_UartPutString("  -> ID request from ");
            put_hex(id >> 24);
            put_hex(id >> 16);
            put_hex(id >> 8);
            put_hex(id);
            ...
            local_buf[2] = id >> 8;
            local_buf[3] = id;
            local_buf[4] = module_id;
            mcp2515_write_array(TXB0D0, local_buf, 5);
            set_can_id_std(TXB0SIDH, MISSION_CONTROL_ID);
            mcp2515_write(TXB0DLC, 5);
        ...
        default:
            SERIAL_UartPutString("  xx Unknown admin request");
        }
    }
}
...
int main()
{
    ...
    for(;;)
    {
        ...
        if (!queue_empty()) {
            ...
            handle_can_message(eid, extended, is_remote, len, data);
        }
    }
}
