...
void init_can()
{
    ...
    // Configure baud rate
    // Configured for 1 MHz baud rate with 20 MHz fosc. See Section 5 of the MCP2515 datasheet.
    //
    // The Nominal Bit Rate (NBR) is defined in the CAN specifiation as the
    // number of bits per second transmitted by an ideal transmitter with
    // no resynchronization. From this, we set up Nominal Bit Time (NBT) as
    //
    // NBT = 1 / NBR
    //
    // The NBT consists of non-overlapping segments as the following.
    // Each segment consts of integer units called Time Quanta (TQ).
    //
    //                           sample point
    //                           v
    ...
    //
    // where
    //   SyncSeg : Synchronization Segment - Used to synchronize the nodes on the bus, always 1TQ
    //   PropSeg : Propagation Segment - To conpensate for physical delays between nodes, 1-8 TQ
    //   PS1     : Phase Segment 1- Used to compensate for edge phase errors on the bus, 1-8 TQ
    //   PS2     : Phase Segment 1- Used to compensate for edge phase errors on the bus, 2-8 TQ
    //
    // The TQ is calculated as following
    //   TQ = 2 * BRP / fosc
    // where BRP is Baud Rate Prescaler.
    //
    // Here we set 1 to BRP, then TQ is 2 / fosc = 1 / 10M that makes NBT = 10 TQ
    //
    // Constraints for the time segments are:
    //
    //   PropSeg + PS1 >= PS2
    //   PropSeg + PS1 >= Tdelay
    //   PS2 > SJW
    //
    // where SJW is Synchronization Jump Width that are used for adjusting the bit timing.
    //
    //  Typically, the sampling of the bit should take place at about 60-70% of the bit time.
    //  Also, typically, the Tdelay is 1-2 TQ.
    //
    // We set the parameters as the following
    //  SJW = 1 TQ
    //  PropSeg = 2 TQ
    //  PS1 = 3 TQ
    //  PS2 = 4 TQ
